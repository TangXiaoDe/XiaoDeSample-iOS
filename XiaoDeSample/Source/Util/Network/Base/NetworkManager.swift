...
extension NetworkManager {
    ...
    public func request<T: RequestInfoProtocol>(requestInfo: T, complete: @escaping (_ result: RequestResult<T>) -> Void) -> DataRequest {
        ...
        return AF.request(result.path, method: requestInfo.method, parameters: requestInfo.parameter, encoding: result.encoding, headers: result.headers).responseJSON {  [unowned self] response in
            // 日志
            ...
            case .failure(let error):
                // 超时处理
                if let error: NSError = error as NSError?, error.domain == NSURLErrorDomain && error.code == NSURLErrorTimedOut {
            ...
            case .success(let data):
                ...
                // Token更新
                if let token = response.allHeaderFields["Authorization"] as? String, let accountInfo = AccountManager.share.currentAccountInfo {
                ...
                if let model = Mapper<RequestResultModel<T>>().map(JSONObject: data) {
                    resultModel = model
                } else {
                    resultModel = RequestResultModel<T>.init()
                }
                resultModel.statusCode = statusCode
                resultModel.sourceData = data
                // 请求结果处理
                DispatchQueue.main.async(execute: {
                    ...
                    case StatusCode.requestSuccess.rawValue:
                        // 200 - 请求成功
                        if let _ = Mapper<RequestResultModel<T>>().map(JSONObject: data) {
...
extension NetworkManager {
    ...
    fileprivate func requestInfoProcess<T: RequestInfoProtocol>(_ requestInfo: T, _ authorization: String?) -> (headers: HTTPHeaders, path: String, encoding: ParameterEncoding) {
        ...
        if let authorization = self.authorization {
            ...
            coustomHeaders.update(name: "Authorization", value: token)
        }
